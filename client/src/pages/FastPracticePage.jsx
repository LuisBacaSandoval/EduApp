...
const FastPracticePage = () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.detail ||
          "Ocurrió un problema al generar el contenido."
      );
      setSelectedFile(null);
    } finally {
      setLoading(false);
  ...
  const handleShare = async () => {
    const code = uuidv4();
    const link = `${window.location.origin}/practice-students?code=${code}`;
    localStorage.setItem("quizCode", code);
    try {
      await navigator.clipboard.writeText(link);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000); // Oculta feedback después de 2s
      ...
      if (error) {
        ...
        console.log("📌 Preguntas guardadas en BD:", data);
      }
    } catch (err) {
  ...
  const viewAnswers = async () => {
    ...
    if (!code) {
      alert("Primero debes generar y compartir el quiz.");
      return;
    }

    setShowResults((prev) => !prev); // Alternar vista

    if (!showResults) {
      const { data, error } = await supabase
        .from("student_responses")
        .select("name, correct_answers, incorrect_answers, score")
        .eq("code", code);
          ...
          `}
            ...
            <div className="flex justify-center">
              <button
                type="submit"
                disabled={loading}
                className={`bg-indigo-600 text-white px-8 py-3 rounded-xl w-1/2 transition-colors cursor-pointer ${
                  loading
            ...
            {quizData.questions.map((question) => {
              ...
              const userAnswerText =
                userAnswerIndex !== undefined
                  ? question.possibleAnswers[userAnswerIndex]
                  : "Sin responder";
  ...
  return (
    <div className="flex flex-row h-screen">
      ...
      <div className="flex flex-4 p-8 flex-col bg-white overflow-hidden">
        ...
        {showResults && (
          <div className="my-6 bg-gray-50 border rounded-lg p-4 min-h-64 max-h-128 overflow-y-auto">
            ...
            {studentResponses.length === 0 ? (
              ...
              <ul className="space-y-3">
                {studentResponses.map((resp, index) => (
                  <li
                    key={index}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Correctas: {resp.correct_answers} | Incorrectas:{" "}
                        {resp.incorrect_answers}
                      </p>
                    </div>
                    <span
                      className={`font-bold ${
                        resp.score >= 70 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {resp.score}%
                    </span>
                ...
                <div className="space-y-3">
                  {question.possibleAnswers.map((answer, answerIndex) => (
                    <div
                      key={answerIndex}
                      onClick={() =>
                        handleAnswerChange(question.id, answerIndex)
                      }
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded-full border-2 mt-0.5 mr-3 flex items-center justify-center shrink-0 ${
                          answers[question.id] === answerIndex
...
export default FastPracticePage;
